...
function addKeyValuePairs(dictionary, entries) {
  ...
  let rejected = 0
  entries.forEach((e) => {
    if (dictionary.has(e[0])) {
      ++rejected
    } else {
      dictionary.set(e[0], e[1])
      ++added
    }
  })
  return {dictionary, 'added': added, 'rejected': rejected}
}
...
function transformDictionary(dictionary) {
 ...
 let values = []
 let entries = []
 let d1 = dictionary.keys()
 for (const i of d1) {
  keys.push(i)
 }
 let d2 = dictionary.values()
 for (const i of d2) {
  values.push(i)
 }
 let d3 = dictionary.entries()
 for (const i of d3) {
  entries.push(i)
 }
...
function convertDictionaryToSet(dictionary) {
  ...
  let s = new Set()
  let d = dictionary.keys()
  for (const [k, v] of dictionary) {
    if (typeof(k) == 'string') {
      s.add(k.toUpperCase())
    }else if(typeof(k) == 'number'){
      let temp = k + 1 //++k чогось не спрацювало
      s.add(temp)
    }
  }
  return s
